...
class TabunganController extends Controller
{
    ...
    public function update(Request $request, Tabungan $tabungan, Push $push, Firebase $firebase)
    {
        ...
        $notification = 'Permeritahuan: Admin ' . auth()->user()->name . '(' . auth()->user()->email . ') sedang menanggapi pengajuan tabungan dari pengguna ' . $tabungan->customer->user->name . '(' . $tabungan->customer->user->email . '): ' . $request->description . ($request->reply ? strip_tags('#' . $request->reply) : '');
